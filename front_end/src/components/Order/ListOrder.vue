  ...
  <div>
    ...
    <table class="table table-hover">
      ...
      <tr v-for="item in items" :key="item._id">
        ...
        <td class="table-primary" v-if="update" @click="updateState(item._id)">
          <h4>{{ item.State }}</h4>
        </td>
        <td class="table-primary" v-if="!update">
          <h4>{{ item.State }}</h4>
        </td>
        <td class="table-primary">
          <h4>{{ item.Location }}</h4>
...
export default {
  ...
  methods: {
    async updateState(id) {
      ...
      let request = {
        url: `http://localhost:3000/Order/UpdateState/${id}`, // should be replaced after going to production with domain URL
      ...
      console.log("esponse.data");
        ...
        this.$router.push({ name: 'UpdateState' })
